...
export function runGame() {
    ...
    function move(direction) {
        const finalPositions = moves.reduce(
            (position, moveValue) => {
                if (moveValue === 'forward') return { lane: position.lane + 1, column: position.column };
                if (moveValue === 'backward') return { lane: position.lane - 1, column: position.column };
                if (moveValue === 'left') return { lane: position.lane, column: position.column - 1 };
                if (moveValue === 'right') return { lane: position.lane, column: position.column + 1 };
                return { lane: position.lane, column: position.column };
            },
    ...
    this.retryBt &&
        this.retryBt.addEventListener('click', () => {
            lanes.forEach(lane => scene.remove(lane.mesh));
            initaliseValues();
            endDOM.style.visibility = 'hidden';
        });

    this.forwardBt && this.forwardBt.addEventListener('click', () => move('forward'));

    this.backwardBt && this.backwardBt.addEventListener('click', () => move('backward'));
