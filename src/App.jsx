                ...
                <div className={['chat-room', window.deviceInfo.isPc ? '' : 'phone'].join(' ')}>
                    <div className="usr-info">
                        <img
                            ...
                            src="./src/hanbag.png"
                            onClick={() => {
                                window.open('./src/hanbag.png');
                            }}
                        ></img>
                        <div className="right-info">
                            <div className="name">精神小伙杰尼🐢</div>
                            <div className="sign">
                                {this.state.robotTyping ? '正在输入中...' : '大🍔批，真🦷么真🍔批'}
                            </div>
                        </div>
                    </div>
                    <div className="chat-info">
                        <div className="send-info" ref="message">
                            {this.isAdmin && (
                                <div className="admin-power">
                                    {menuList.map(item => (
                                        <a style={{ padding: '0 10px' }} href={item.url}>
                                            {item.name}
                                        </a>
                                    ))}
                                </div>
                            ...
                            {messageList.map((item, index) => {
                                ...
                                return (
                                    <div key={index} className={[sender, 'message'].join(' ')}>
                                        <img
                                            src={sender === 'robot' ? './src/hanbag.png' : './src/daidais.png'}
                                            onClick={() => {
                                                window.open(
                                                    `${sender === 'robot' ? './src/hanbag.png' : './src/daidais.png'}`
                                                );
                                            }}
                                        />
